...
public class CategoryController {
    ...
    @GetMapping
    public ResponseEntity<Page<CategoryShortResponse>> getAllCategories(@Parameter(name = "pagination parameter",
            description = "Pagination and sorting parameters", example = PAGINATION_EXAMPLE)
                                                                            @PageableDefault(sort = "id", direction = Sort.Direction.ASC) Pageable pageable) {
        return ResponseEntity.ok(categoryService.getAllShort(pageable));
    }
    ...
    @ApiResponse(responseCode = "404", description = "Category not found")
    public ResponseEntity<CategoryResponse> updateCategory(@Parameter(description = "ID of category", required = true) @PathVariable("id") Long id,
